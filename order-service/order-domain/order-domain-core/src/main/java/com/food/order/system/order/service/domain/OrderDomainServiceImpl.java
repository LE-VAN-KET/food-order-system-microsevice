...
public class OrderDomainServiceImpl implements OrderDomainService{
    private static final String UTC = "GMT+7";
    @Override
    ...
    private void setOrderProductInformation(Order order, Restaurant restaurant) {
        Map<ProductId, Product> restaurantProductHash = restaurant.getProducts().stream()
                .collect(Collectors.toMap(Product::getId, Function.identity()));
        order.getItems().forEach(orderItem -> {
            Product currentProduct = orderItem.getProduct();
            ProductId id = currentProduct.getId();
            if (restaurantProductHash.get(id) != null) {
                Product restaurantProduct = restaurantProductHash.get(id);
